...
//
// A delay is a courtesy to the *network*; when a fake/mock response has already
// been stashed (the mock path never hits the wire), there is nothing to be polite
// to, so we skip it — otherwise mocked tests would sleep the full real delay.
...
export async function delayMiddleware(pending: PendingRequest): Promise<void> {
  if (pending.hasFakeResponse()) return;
